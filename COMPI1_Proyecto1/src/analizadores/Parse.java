
//----------------------------------------------------
// The following code was generated by CUP v0.11b 20160615 (GIT 4ac7450)
//----------------------------------------------------

package analizadores;

import java_cup.runtime.*;
import java_cup.runtime.XMLElement;

/** CUP v0.11b 20160615 (GIT 4ac7450) generated parser.
  */
@SuppressWarnings({"rawtypes"})
public class Parse extends java_cup.runtime.lr_parser {

 public final Class getSymbolContainer() {
    return sym.class;
}

  /** Default constructor. */
  @Deprecated
  public Parse() {super();}

  /** Constructor which sets the default scanner. */
  @Deprecated
  public Parse(java_cup.runtime.Scanner s) {super(s);}

  /** Constructor which sets the default scanner. */
  public Parse(java_cup.runtime.Scanner s, java_cup.runtime.SymbolFactory sf) {super(s,sf);}

  /** Production table. */
  protected static final short _production_table[][] = 
    unpackFromStrings(new String[] {
    "\000\014\000\002\002\004\000\002\002\003\000\002\002" +
    "\003\000\002\003\003\000\002\004\004\000\002\004\005" +
    "\000\002\004\005\000\002\004\005\000\002\004\005\000" +
    "\002\004\005\000\002\004\003\000\002\004\003" });

  /** Access to production table. */
  public short[][] production_table() {return _production_table;}

  /** Parse-action table. */
  protected static final short[][] _action_table = 
    unpackFromStrings(new String[] {
    "\000\025\000\014\003\006\004\010\007\011\012\007\013" +
    "\013\001\002\000\004\002\000\001\002\000\004\002\027" +
    "\001\002\000\004\002\uffff\001\002\000\016\002\ufff7\005" +
    "\ufff7\006\ufff7\007\ufff7\010\ufff7\011\ufff7\001\002\000\012" +
    "\004\010\007\011\012\007\013\013\001\002\000\012\004" +
    "\010\007\011\012\007\013\013\001\002\000\014\002\ufffe" +
    "\006\014\007\015\010\017\011\016\001\002\000\016\002" +
    "\ufff6\005\ufff6\006\ufff6\007\ufff6\010\ufff6\011\ufff6\001\002" +
    "\000\012\004\010\007\011\012\007\013\013\001\002\000" +
    "\012\004\010\007\011\012\007\013\013\001\002\000\012" +
    "\004\010\007\011\012\007\013\013\001\002\000\012\004" +
    "\010\007\011\012\007\013\013\001\002\000\016\002\ufffa" +
    "\005\ufffa\006\ufffa\007\ufffa\010\ufffa\011\ufffa\001\002\000" +
    "\016\002\ufff9\005\ufff9\006\ufff9\007\ufff9\010\ufff9\011\ufff9" +
    "\001\002\000\016\002\ufffc\005\ufffc\006\ufffc\007\ufffc\010" +
    "\017\011\016\001\002\000\016\002\ufffb\005\ufffb\006\ufffb" +
    "\007\ufffb\010\017\011\016\001\002\000\016\002\ufffd\005" +
    "\ufffd\006\ufffd\007\ufffd\010\ufffd\011\ufffd\001\002\000\014" +
    "\005\026\006\014\007\015\010\017\011\016\001\002\000" +
    "\016\002\ufff8\005\ufff8\006\ufff8\007\ufff8\010\ufff8\011\ufff8" +
    "\001\002\000\004\002\001\001\002" });

  /** Access to parse-action table. */
  public short[][] action_table() {return _action_table;}

  /** <code>reduce_goto</code> table. */
  protected static final short[][] _reduce_table = 
    unpackFromStrings(new String[] {
    "\000\025\000\010\002\004\003\003\004\011\001\001\000" +
    "\002\001\001\000\002\001\001\000\002\001\001\000\002" +
    "\001\001\000\004\004\024\001\001\000\004\004\023\001" +
    "\001\000\002\001\001\000\002\001\001\000\004\004\022" +
    "\001\001\000\004\004\021\001\001\000\004\004\020\001" +
    "\001\000\004\004\017\001\001\000\002\001\001\000\002" +
    "\001\001\000\002\001\001\000\002\001\001\000\002\001" +
    "\001\000\002\001\001\000\002\001\001\000\002\001\001" +
    "" });

  /** Access to <code>reduce_goto</code> table. */
  public short[][] reduce_table() {return _reduce_table;}

  /** Instance of action encapsulation class. */
  protected CUP$Parse$actions action_obj;

  /** Action encapsulation object initializer. */
  protected void init_actions()
    {
      action_obj = new CUP$Parse$actions(this);
    }

  /** Invoke a user supplied parse action. */
  public java_cup.runtime.Symbol do_action(
    int                        act_num,
    java_cup.runtime.lr_parser parser,
    java.util.Stack            stack,
    int                        top)
    throws java.lang.Exception
  {
    /* call code in generated class */
    return action_obj.CUP$Parse$do_action(act_num, parser, stack, top);
  }

  /** Indicates start state. */
  public int start_state() {return 0;}
  /** Indicates start production. */
  public int start_production() {return 0;}

  /** <code>EOF</code> Symbol index. */
  public int EOF_sym() {return 0;}

  /** <code>error</code> Symbol index. */
  public int error_sym() {return 1;}



    public void syntax_error(Symbol s) {
        System.out.println("Error sintáctico: " + s.value);
    }

    public void unrecover_syntax_error(Symbol s) throws java.lang.Exception {
        System.out.println("Error sintáctico unrecovered: " + s.value);
    }


/** Cup generated class to encapsulate user supplied action code.*/
@SuppressWarnings({"rawtypes", "unchecked", "unused"})
class CUP$Parse$actions {
  private final Parse parser;

  /** Constructor */
  CUP$Parse$actions(Parse parser) {
    this.parser = parser;
  }

  /** Method 0 with the actual generated action code for actions 0 to 300. */
  public final java_cup.runtime.Symbol CUP$Parse$do_action_part00000000(
    int                        CUP$Parse$act_num,
    java_cup.runtime.lr_parser CUP$Parse$parser,
    java.util.Stack            CUP$Parse$stack,
    int                        CUP$Parse$top)
    throws java.lang.Exception
    {
      /* Symbol object for return from actions */
      java_cup.runtime.Symbol CUP$Parse$result;

      /* select the action based on the action number */
      switch (CUP$Parse$act_num)
        {
          /*. . . . . . . . . . . . . . . . . . . .*/
          case 0: // $START ::= ini EOF 
            {
              Object RESULT =null;
		int start_valleft = ((java_cup.runtime.Symbol)CUP$Parse$stack.elementAt(CUP$Parse$top-1)).left;
		int start_valright = ((java_cup.runtime.Symbol)CUP$Parse$stack.elementAt(CUP$Parse$top-1)).right;
		Object start_val = (Object)((java_cup.runtime.Symbol) CUP$Parse$stack.elementAt(CUP$Parse$top-1)).value;
		RESULT = start_val;
              CUP$Parse$result = parser.getSymbolFactory().newSymbol("$START",0, ((java_cup.runtime.Symbol)CUP$Parse$stack.elementAt(CUP$Parse$top-1)), ((java_cup.runtime.Symbol)CUP$Parse$stack.peek()), RESULT);
            }
          /* ACCEPT */
          CUP$Parse$parser.done_parsing();
          return CUP$Parse$result;

          /*. . . . . . . . . . . . . . . . . . . .*/
          case 1: // ini ::= instruction 
            {
              Object RESULT =null;
		int aleft = ((java_cup.runtime.Symbol)CUP$Parse$stack.peek()).left;
		int aright = ((java_cup.runtime.Symbol)CUP$Parse$stack.peek()).right;
		Object a = (Object)((java_cup.runtime.Symbol) CUP$Parse$stack.peek()).value;
		 System.out.println("Result: " + a); 
              CUP$Parse$result = parser.getSymbolFactory().newSymbol("ini",0, ((java_cup.runtime.Symbol)CUP$Parse$stack.peek()), ((java_cup.runtime.Symbol)CUP$Parse$stack.peek()), RESULT);
            }
          return CUP$Parse$result;

          /*. . . . . . . . . . . . . . . . . . . .*/
          case 2: // ini ::= error 
            {
              Object RESULT =null;

              CUP$Parse$result = parser.getSymbolFactory().newSymbol("ini",0, ((java_cup.runtime.Symbol)CUP$Parse$stack.peek()), ((java_cup.runtime.Symbol)CUP$Parse$stack.peek()), RESULT);
            }
          return CUP$Parse$result;

          /*. . . . . . . . . . . . . . . . . . . .*/
          case 3: // instruction ::= expression 
            {
              Object RESULT =null;
		int aleft = ((java_cup.runtime.Symbol)CUP$Parse$stack.peek()).left;
		int aright = ((java_cup.runtime.Symbol)CUP$Parse$stack.peek()).right;
		Double a = (Double)((java_cup.runtime.Symbol) CUP$Parse$stack.peek()).value;
		RESULT = a;
              CUP$Parse$result = parser.getSymbolFactory().newSymbol("instruction",1, ((java_cup.runtime.Symbol)CUP$Parse$stack.peek()), ((java_cup.runtime.Symbol)CUP$Parse$stack.peek()), RESULT);
            }
          return CUP$Parse$result;

          /*. . . . . . . . . . . . . . . . . . . .*/
          case 4: // expression ::= MINUS expression 
            {
              Double RESULT =null;
		int leftleft = ((java_cup.runtime.Symbol)CUP$Parse$stack.peek()).left;
		int leftright = ((java_cup.runtime.Symbol)CUP$Parse$stack.peek()).right;
		Double left = (Double)((java_cup.runtime.Symbol) CUP$Parse$stack.peek()).value;
		RESULT = 0-left;
              CUP$Parse$result = parser.getSymbolFactory().newSymbol("expression",2, ((java_cup.runtime.Symbol)CUP$Parse$stack.elementAt(CUP$Parse$top-1)), ((java_cup.runtime.Symbol)CUP$Parse$stack.peek()), RESULT);
            }
          return CUP$Parse$result;

          /*. . . . . . . . . . . . . . . . . . . .*/
          case 5: // expression ::= expression MINUS expression 
            {
              Double RESULT =null;
		int leftleft = ((java_cup.runtime.Symbol)CUP$Parse$stack.elementAt(CUP$Parse$top-2)).left;
		int leftright = ((java_cup.runtime.Symbol)CUP$Parse$stack.elementAt(CUP$Parse$top-2)).right;
		Double left = (Double)((java_cup.runtime.Symbol) CUP$Parse$stack.elementAt(CUP$Parse$top-2)).value;
		int rightleft = ((java_cup.runtime.Symbol)CUP$Parse$stack.peek()).left;
		int rightright = ((java_cup.runtime.Symbol)CUP$Parse$stack.peek()).right;
		Double right = (Double)((java_cup.runtime.Symbol) CUP$Parse$stack.peek()).value;
		RESULT = left-right;
              CUP$Parse$result = parser.getSymbolFactory().newSymbol("expression",2, ((java_cup.runtime.Symbol)CUP$Parse$stack.elementAt(CUP$Parse$top-2)), ((java_cup.runtime.Symbol)CUP$Parse$stack.peek()), RESULT);
            }
          return CUP$Parse$result;

          /*. . . . . . . . . . . . . . . . . . . .*/
          case 6: // expression ::= expression ADD expression 
            {
              Double RESULT =null;
		int leftleft = ((java_cup.runtime.Symbol)CUP$Parse$stack.elementAt(CUP$Parse$top-2)).left;
		int leftright = ((java_cup.runtime.Symbol)CUP$Parse$stack.elementAt(CUP$Parse$top-2)).right;
		Double left = (Double)((java_cup.runtime.Symbol) CUP$Parse$stack.elementAt(CUP$Parse$top-2)).value;
		int rightleft = ((java_cup.runtime.Symbol)CUP$Parse$stack.peek()).left;
		int rightright = ((java_cup.runtime.Symbol)CUP$Parse$stack.peek()).right;
		Double right = (Double)((java_cup.runtime.Symbol) CUP$Parse$stack.peek()).value;
		RESULT = left+right;
              CUP$Parse$result = parser.getSymbolFactory().newSymbol("expression",2, ((java_cup.runtime.Symbol)CUP$Parse$stack.elementAt(CUP$Parse$top-2)), ((java_cup.runtime.Symbol)CUP$Parse$stack.peek()), RESULT);
            }
          return CUP$Parse$result;

          /*. . . . . . . . . . . . . . . . . . . .*/
          case 7: // expression ::= expression TIMES expression 
            {
              Double RESULT =null;
		int leftleft = ((java_cup.runtime.Symbol)CUP$Parse$stack.elementAt(CUP$Parse$top-2)).left;
		int leftright = ((java_cup.runtime.Symbol)CUP$Parse$stack.elementAt(CUP$Parse$top-2)).right;
		Double left = (Double)((java_cup.runtime.Symbol) CUP$Parse$stack.elementAt(CUP$Parse$top-2)).value;
		int rightleft = ((java_cup.runtime.Symbol)CUP$Parse$stack.peek()).left;
		int rightright = ((java_cup.runtime.Symbol)CUP$Parse$stack.peek()).right;
		Double right = (Double)((java_cup.runtime.Symbol) CUP$Parse$stack.peek()).value;
		RESULT = left*right;
              CUP$Parse$result = parser.getSymbolFactory().newSymbol("expression",2, ((java_cup.runtime.Symbol)CUP$Parse$stack.elementAt(CUP$Parse$top-2)), ((java_cup.runtime.Symbol)CUP$Parse$stack.peek()), RESULT);
            }
          return CUP$Parse$result;

          /*. . . . . . . . . . . . . . . . . . . .*/
          case 8: // expression ::= expression DIVIDE expression 
            {
              Double RESULT =null;
		int leftleft = ((java_cup.runtime.Symbol)CUP$Parse$stack.elementAt(CUP$Parse$top-2)).left;
		int leftright = ((java_cup.runtime.Symbol)CUP$Parse$stack.elementAt(CUP$Parse$top-2)).right;
		Double left = (Double)((java_cup.runtime.Symbol) CUP$Parse$stack.elementAt(CUP$Parse$top-2)).value;
		int rightleft = ((java_cup.runtime.Symbol)CUP$Parse$stack.peek()).left;
		int rightright = ((java_cup.runtime.Symbol)CUP$Parse$stack.peek()).right;
		Double right = (Double)((java_cup.runtime.Symbol) CUP$Parse$stack.peek()).value;
		RESULT = left/right;
              CUP$Parse$result = parser.getSymbolFactory().newSymbol("expression",2, ((java_cup.runtime.Symbol)CUP$Parse$stack.elementAt(CUP$Parse$top-2)), ((java_cup.runtime.Symbol)CUP$Parse$stack.peek()), RESULT);
            }
          return CUP$Parse$result;

          /*. . . . . . . . . . . . . . . . . . . .*/
          case 9: // expression ::= PARLEFT expression PARRIGHT 
            {
              Double RESULT =null;
		int expleft = ((java_cup.runtime.Symbol)CUP$Parse$stack.elementAt(CUP$Parse$top-1)).left;
		int expright = ((java_cup.runtime.Symbol)CUP$Parse$stack.elementAt(CUP$Parse$top-1)).right;
		Double exp = (Double)((java_cup.runtime.Symbol) CUP$Parse$stack.elementAt(CUP$Parse$top-1)).value;
		RESULT = exp;
              CUP$Parse$result = parser.getSymbolFactory().newSymbol("expression",2, ((java_cup.runtime.Symbol)CUP$Parse$stack.elementAt(CUP$Parse$top-2)), ((java_cup.runtime.Symbol)CUP$Parse$stack.peek()), RESULT);
            }
          return CUP$Parse$result;

          /*. . . . . . . . . . . . . . . . . . . .*/
          case 10: // expression ::= NUMBER 
            {
              Double RESULT =null;
		int numleft = ((java_cup.runtime.Symbol)CUP$Parse$stack.peek()).left;
		int numright = ((java_cup.runtime.Symbol)CUP$Parse$stack.peek()).right;
		String num = (String)((java_cup.runtime.Symbol) CUP$Parse$stack.peek()).value;
		RESULT = new Double(num);
              CUP$Parse$result = parser.getSymbolFactory().newSymbol("expression",2, ((java_cup.runtime.Symbol)CUP$Parse$stack.peek()), ((java_cup.runtime.Symbol)CUP$Parse$stack.peek()), RESULT);
            }
          return CUP$Parse$result;

          /*. . . . . . . . . . . . . . . . . . . .*/
          case 11: // expression ::= DECIMAL 
            {
              Double RESULT =null;
		int numleft = ((java_cup.runtime.Symbol)CUP$Parse$stack.peek()).left;
		int numright = ((java_cup.runtime.Symbol)CUP$Parse$stack.peek()).right;
		String num = (String)((java_cup.runtime.Symbol) CUP$Parse$stack.peek()).value;
		RESULT = new Double(num);
              CUP$Parse$result = parser.getSymbolFactory().newSymbol("expression",2, ((java_cup.runtime.Symbol)CUP$Parse$stack.peek()), ((java_cup.runtime.Symbol)CUP$Parse$stack.peek()), RESULT);
            }
          return CUP$Parse$result;

          /* . . . . . .*/
          default:
            throw new Exception(
               "Invalid action number "+CUP$Parse$act_num+"found in internal parse table");

        }
    } /* end of method */

  /** Method splitting the generated action code into several parts. */
  public final java_cup.runtime.Symbol CUP$Parse$do_action(
    int                        CUP$Parse$act_num,
    java_cup.runtime.lr_parser CUP$Parse$parser,
    java.util.Stack            CUP$Parse$stack,
    int                        CUP$Parse$top)
    throws java.lang.Exception
    {
              return CUP$Parse$do_action_part00000000(
                               CUP$Parse$act_num,
                               CUP$Parse$parser,
                               CUP$Parse$stack,
                               CUP$Parse$top);
    }
}

}
